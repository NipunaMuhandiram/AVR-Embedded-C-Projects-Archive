...
ISR(INT0_vect) {
	...
	if (!(PIND & (1 << PD2))) {  // Check if button is still pressed
		counter = (counter + 1) % 16;  // Increment counter and wrap around at 16 (0-15)
		PORTB = (counter << 3);  // Display the counter value on PB3, PB4, PB5, PB6
	}
}

int main(void) {
	// Set PB3-PB6 as output for displaying the counter value
	DDRB = (1 << PB3) | (1 << PB4) | (1 << PB5) | (1 << PB6);  // Set PB3-PB6 as output
